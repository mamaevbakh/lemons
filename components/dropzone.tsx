  ...
  return (
    <DropzoneContext.Provider value={{ ...restProps }}>
      <div
        {...getRootProps({
          className: cn(
            'border-2 border-border rounded-lg p-6 text-center bg-background transition-colors duration-300 text-foreground',
            className,
...
const DropzoneContent = ({
  ...
  const { files, setFiles, onUpload, loading, successes, errors, maxFileSize, maxFiles } =
    useDropzoneContext()
        ...
        return (
          ...
          >
            {file.type.startsWith('image/') ? (
              ...
              <div className="h-10 w-10 rounded border overflow-hidden shrink-0 bg-muted flex items-center justify-center">
                <video src={file.preview} muted playsInline className="h-full w-full object-cover" />
              </div>
            ) : (
