...
type UseSupabaseUploadOptions = {
   ...
   *
   * If provided, `useSupabaseUpload` will call this instead of uploading
...
const useSupabaseUpload = (options: UseSupabaseUploadOptions) => {
  ...
  const onDrop = (acceptedFiles: File[], fileRejections: FileRejection[]) => {
    ...
    // If the number of files doesn't exceed maxFiles, remove the dropzone-level
    // 'too-many-files' error from each file.
    if (newFiles.length <= maxFiles) {
      newFiles = newFiles.map((file) => {
        if (file.errors.some((e) => e.code === 'too-many-files')) {
          ;(file as FileWithPreview).errors = file.errors.filter((e) => e.code !== 'too-many-files')
        }
        return file
      })
    }
